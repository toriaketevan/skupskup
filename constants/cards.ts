...
import NewLetterSvg    from '../assets/cards/new-letter.svg';
import WordsSvg        from '../assets/cards/words.svg';
import LetterWritingSvg from '../assets/cards/letter-writing.svg';
import WordReadingSvg  from '../assets/cards/word-reading.svg';

export type FieldDef = { key: string; label: string; multiline?: boolean };

export type PageDraft = { text: string; image: string };

export const CARD_TYPES: { key: Card; label: string; emoji: string; color: string; svg?: any }[] = [
  { key: 'new_letter',     label: 'ახალი ასო',           emoji: '🔤', color: '#6366F1', svg: NewLetterSvg     },
  { key: 'sound_story',    label: 'სიტყვები',            emoji: '🔊', color: '#EC4899', svg: WordsSvg         },
  { key: 'letter_writing', label: 'დავწეროთ ასო',         emoji: '✍️',  color: '#F59E0B', svg: LetterWritingSvg },
  { key: 'word_reading',   label: 'წავიკითხოთ სიტყვები', emoji: '📖', color: '#10B981', svg: WordReadingSvg   },
  { key: 'book',           label: 'წიგნი',               emoji: '📚', color: '#3B82F6' },
  { key: 'letter_review',  label: 'გავიმეოროთ ასოები',   emoji: '🔁', color: '#8B5CF6' },
  { key: 'alphabet_song',  label: 'ანბანის სიმღერა',     emoji: '🎵', color: '#EF4444' },
  { key: 'quick_check',    label: 'შავაჯამოთ გაკვეთილი', emoji: '✅', color: '#14B8A6' },
  { key: 'comprehension',  label: 'გავიაზროთ წაკითხული', emoji: '🧠', color: '#F97316' },
];
